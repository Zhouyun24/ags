...
<div class="flex flex-col min-h-screen bg-slate-50 pb-10">
    ...
    <div class="px-6 mt-6 w-full max-w-md mx-auto">
        <form method="POST" action="{{ route('dosen.hasil-bimbingan.update', $hasil->id_hasil) }}" class="space-y-6">
            @csrf
            @method('PATCH')
            <div class="rounded-xl border border-[#60a5fa] bg-[#e0e7ff] p-4 shadow-sm">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h3 class="font-bold text-slate-900 text-[15px]">{{ $hasil->jadwal_bimbingan?->mahasiswa?->pengguna?->nama ?? '-' }}</h3>
                        <div class="text-[13px] text-slate-500 mt-0.5 leading-tight">
                            <p>NIM: {{ $hasil->jadwal_bimbingan?->mahasiswa?->nim ?? $hasil->jadwal_bimbingan?->nim }}</p>
                            <p>{{ $hasil->jadwal_bimbingan?->tanggal_jadwal ? \Illuminate\Support\Carbon::parse($hasil->jadwal_bimbingan->tanggal_jadwal)->format('d/m/Y') : '-' }} &bull; {{ $hasil->jadwal_bimbingan?->jam_jadwal ? \Illuminate\Support\Carbon::parse($hasil->jadwal_bimbingan->jam_jadwal)->format('H.i') . ' WIB' : '-' }}</p>
                        </div>
                    </div>
                </div>
            </div>
