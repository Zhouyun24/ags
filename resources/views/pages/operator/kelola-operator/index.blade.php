...
@section('layouts')
@php
    $jumlahOperator = $operators->count();
@endphp
    ...
    <div class="relative overflow-hidden bg-gradient-to-br from-[#8B5CF6] via-[#7C3AED] to-[#6D28D9] px-5 pb-12 pt-10">
        <div class="flex items-start justify-between gap-3">
            <div>
                ...
                <p class="mt-1 font-inter text-xs text-white">{{ $jumlahOperator }} Operator terdaftar</p>
            </div>
            <a href="{{ route('operator.kelola-operator.create') }}"
                class="flex shrink-0 items-center gap-1.5 rounded-lg bg-gradient-to-br from-[#8B5CF6] via-[#7C3AED] to-[#6D28D9] px-4 py-2.5 font-inter text-xs font-semibold text-white shadow-[0px_4px_12px_0px_#00000030] border border-[#BC95D4]">
    ...
    <div class="px-5 pt-6">
        ...
        <div class="flex flex-col gap-4">
            @forelse ($operators as $operator)
                <div x-show="cari === '' || '{{ strtolower($operator->id_operator . ' ' . ($operator->pengguna->nama ?? '')) }}'.includes(cari.toLowerCase())" class="mb-4 rounded-xl border border-slate-100 bg-white p-4 shadow-[0px_4px_16px_0px_#0F172A14]">
                    <div class="mb-4 flex items-start gap-4">
                        <img src="{{ $operator->pengguna?->foto_profil ? asset('storage/' . $operator->pengguna->foto_profil) : 'https://ui-avatars.com/api/?name=' . urlencode($operator->pengguna?->nama ?? 'O') . '&background=random' }}"
                            alt="Profile" class="h-[52px] w-[52px] rounded-full object-cover">
                        <div class="flex-1">
                            <h2 class="font-jakarta text-base font-extrabold text-black">{{ $operator->pengguna?->nama ?? '-' }}</h2>
                            <p class="font-inter text-sm text-slate-500">{{ $operator->id_operator }}</p>
                        </div>
                    </div>

                    <div class="mb-4 grid grid-cols-1 gap-4 rounded-lg bg-slate-50 p-3">
                        <div>
                            <p class="mb-1 font-inter text-[10px] text-slate-400">Email</p>
                            <p class="font-inter text-xs font-semibold text-black">{{ $operator->pengguna?->email }}</p>
                        </div>
                    </div>

                    <div class="grid grid-cols-3 gap-2">
                        <a href="{{ route('operator.kelola-operator.show', $operator->id_pengguna) }}"
                        ...
                        <button type="button" @click="bukaModalHapus('{{ $operator->id_pengguna }}', '{{ addslashes($operator->pengguna?->nama ?? '') }}', '{{ route('operator.kelola-operator.destroy', $operator->id_pengguna) }}')"
            ...
            @empty
                <div class="rounded-xl bg-white py-10 text-center shadow-[0px_4px_16px_0px_#0F172A14]">
                    <p class="font-inter text-sm text-slate-400">Belum ada data operator.</p>
                </div>
            @endforelse
        </div>
    </div>
    ...
    >
        ...
        >
            <div class="flex flex-col items-center text-center">
                ...
                <p class="mt-3 font-jakarta text-base font-extrabold text-black">Hapus Data Operator?</p>
                <p class="mt-1 font-inter text-xs text-slate-500">
                    Data operator <span class="font-semibold text-black" x-text="targetNama"></span> akan dihapus secara permanen dan tidak dapat dikembalikan.
                </p>
            </div>
