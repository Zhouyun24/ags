...
>
    <div class="relative overflow-hidden bg-gradient-to-br from-[#8B5CF6] via-[#7C3AED] to-[#6D28D9] px-5 pb-12 pt-10">
        <div class="flex items-start justify-between gap-3">
            <div>
                <h1 class="font-jakarta text-xl font-extrabold text-white">Manajemen Dosen</h1>
                <p class="mt-1 font-inter text-xs text-white">{{ $jumlahDosen }} Dosen terdaftar</p>
            </div>
            <a href="{{ route('operator.kelola-dosen.create') }}"
                class="flex shrink-0 items-center gap-1.5 rounded-lg bg-gradient-to-br from-[#22C55E] via-[#16A34A] to-[#15803D] px-4 py-2.5 font-inter text-xs font-semibold text-white shadow-[0px_4px_12px_0px_#00000030] hover:bg-[#1D4ED8]">
    ...
    <div class="px-5 pt-6">
        ...
        <div class="flex flex-col gap-4">
            @forelse ($dosens as $dosen)
                <div x-show="cari === '' || '{{ strtolower($dosen->nip . ' ' . ($dosen->pengguna->nama ?? '')) }}'.includes(cari.toLowerCase())" class="mb-4 rounded-xl border border-slate-100 bg-white p-4 shadow-[0px_4px_16px_0px_#0F172A14]">
    ...
    >
        ...
        >
            <div class="flex flex-col items-center text-center">
                ...
                <p class="mt-3 font-jakarta text-base font-extrabold text-black">Hapus Data Dosen?</p>
                <p class="mt-1 font-inter text-xs text-slate-500">
                    Data dosen <span class="font-semibold text-black" x-text="targetNama"></span> akan dihapus secara permanen dan tidak dapat dikembalikan.
                </p>
            </div>
